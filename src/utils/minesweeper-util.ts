...
export default class MinesweeperGame extends GameBase {
  ...
  protected getEmbed(): MessageEmbed {
    return new Discord.MessageEmbed()
      ...
      .setAuthor('Made By: TurkeyDev - modified by Jaegnah')
      .setDescription(this.gameBoardToString())
      .addField(
        this.flagging ? 'Flagging' : 'Clicking',
        this.flagging ? '🚩' : '👆',
        false
      )
      .addField(
        'How To Play:',
        'Click on a square above and visit the url to reveal, or flag the tile!',
        false
      )
  ...
  private showBombs(): void {
    for (let y = 0; y < HEIGHT; y++) {
      for (let x = 0; x < WIDTH; x++) {
        if (bombLocs[y * WIDTH + x]) gameBoard[y * WIDTH + x] = '💣';
      }
    }
  }
